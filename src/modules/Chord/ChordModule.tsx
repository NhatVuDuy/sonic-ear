...
export function ChordModule() {
  ...
  return (
    <div className="flex flex-col gap-4 animate-[fadeUp_.32s_ease_both]">
      <div>
        <div className="font-display italic text-[1rem] text-gray-600">Nhận diện Hợp âm</div>
      ...
      <Card>
        <div className="flex items-center justify-center gap-5 py-3">
          ...
          <div className="flex-1 text-center">
            <div className="font-display text-4xl text-[#4338ca]">{answered ? NOTE_DISPLAY[q.ri] : '?'}</div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {opts.map(k => {
            ...
            return <OptionBtn key={k} state={state} onClick={()=>answer(k)} disabled={answered}>
              <div className="text-[.85rem] text-[#4338ca]">{CHORDS[k].name}</div>
