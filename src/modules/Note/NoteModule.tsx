...
export function NoteModule() {
  ...
  return (
    <div className="flex flex-col gap-4 animate-[fadeUp_.32s_ease_both]">
      <div>
        <div className="font-display italic text-[1rem] text-gray-600">Nhận diện Nốt đơn</div>
        <p className="mt-1 text-[.82rem] text-gray-600">Nghe một nốt và chọn tên nốt đúng</p>
      </div>
      <Card className="text-center">
        <div className="flex flex-col items-center gap-3 py-3">
          <PlayBtn onClick={() => play(q.ns)} isPlaying={isPlaying} />
          <div className="font-display text-[4.5rem] leading-none" style={{ color: answered ? (ok ? 'var(--color-ok)' : 'var(--color-bad)') : '#9ca3af' }}>
            {answered ? q.correct : '?'}
          </div>
        </div>
        <div className="grid grid-cols-4 gap-2">
          {opts.map(n => {
            ...
            return <OptionBtn key={n} state={state} onClick={()=>answer(n)} disabled={answered}>
              <div className="font-display text-[1.05rem] text-[#4338ca]">{n}</div>
            </OptionBtn>
          })}
        </div>
      </Card>
