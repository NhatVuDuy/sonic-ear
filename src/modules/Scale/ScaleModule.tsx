...
export function ScaleModule() {
  ...
  return (
    <div className="flex flex-col gap-4 animate-[fadeUp_.32s_ease_both]">
      <div>
        <div className="font-display italic text-[1rem] text-gray-600">Nhận diện Điệu thức / Gam</div>
          ...
          <div className="flex-1 text-center">
            <div className="flex gap-1.5 flex-wrap justify-center">
              {['I','II','III','IV','V','VI','VII'].map((d,i) => (
                <div key={d} className={`flex h-8 w-8 items-center justify-center rounded-md border font-mono text-[.62rem] font-bold ${i===0?'border-[#4338ca] bg-[rgba(67,56,202,.10)] text-[#4338ca]':'border-gray-300 bg-gray-50 text-gray-500'}`}>
                  {answered && q.sns[i] ? NOTE_DISPLAY[NOTE_NAMES.indexOf(q.sns[i].replace(/\d/,'') as any)] : d}
                </div>
              ))}
            </div>
            <p className="mt-2 font-mono text-[0.7rem] text-gray-600">Nghe lên và xuống</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2">
          {opts.map(k => {
            ...
            return <OptionBtn key={k} state={state} onClick={()=>answer(k)} disabled={answered}>
              <div className="text-[.85rem] text-[#4338ca]">{SCALES[k].name}</div>
